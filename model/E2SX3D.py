...
class E2SX3D(nn.Module):
    def init_backbone(self, arch, num_classes, pretrained=False, train_backbone=True):
    ...
    def __init__(
        self, rgb_arch, flow_arch, num_classes, weights=None, pretrained=False
    ):
        super(E2SX3D, self).__init__()
        self.rgb_arch = rgb_arch
        self.flow_arch = flow_arch
        self.rgbstream = self.init_backbone(rgb_arch, num_classes, pretrained, False)
        self.flowstream = self.init_backbone(flow_arch, num_classes, pretrained, True)

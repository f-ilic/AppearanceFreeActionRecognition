...
def main():
    ...
    train_dataset = DBfactory2Stream(datasetname,datasetname_motionpath, train=True, fold=1, rgbcfg=rgb_cfg, flowcfg=flow_cfg)
    test_dataset = DBfactory2Stream(datasetname,datasetname_motionpath, train=False, fold=1, rgbcfg=rgb_cfg, flowcfg=flow_cfg)

    train_dataloader = DataLoader(train_dataset, batch_size, num_workers=num_workers, shuffle=True, drop_last=True)
    test_dataloader = DataLoader(test_dataset, batch_size, num_workers=num_workers, shuffle=True, drop_last=True)

    # ----------------- Setup Model & Load weights if supplied -----------------
    model = E2SX3D(cfg['rgbarch'], cfg['flowarch'], train_dataset.num_classes, cfg['weights_path'], cfg['pretrained'])
    ...
    with Simulation(sim_name=sim_name, output_root='runs') as sim:
        ...
        for epoch in range(cfg['num_epochs']):
            ...
            trainer.do('train', model, train_dataloader, epoch, criterion, optimizer, writer, log_video=False)
        
            if epoch % 2 == 0 or epoch==cfg['num_epochs']-1:
                trainer.do('test', model, test_dataloader, epoch, criterion, None, writer, log_video=False)
                checkpoint = {'epoch': epoch, 'state_dict': model.state_dict(),'optimizer': optimizer.state_dict()}
                sim.save_pytorch(checkpoint, epoch=epoch)
